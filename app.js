...
async function seatingPage(){
 ...
 const modal=$('[data-table-modal]');$('[data-add-table]').onclick=()=>modal.classList.add('show');$('[data-close-table]').onclick=()=>modal.classList.remove('show');$('[data-table-form]').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);await api('/api/seating/tables',{method:'POST',body:JSON.stringify(Object.fromEntries(f.entries()))});location.reload()}
}
...
async function budgetPage(){
 ...
 const modal=$('[data-budget-modal]');$('[data-open-budget]').onclick=()=>modal.classList.add('show');$('[data-close-budget]').onclick=()=>modal.classList.remove('show');$('[data-budget-form]').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);await api('/api/budget',{method:'POST',body:JSON.stringify(Object.fromEntries(f.entries()))});location.reload()}
}
