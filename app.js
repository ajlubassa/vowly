...
async function eventsPage(){const rows=await api('/api/events'),list=$('[data-event-list]');list.innerHTML=rows.map(e=>`<article class="event-card"><span class="eyebrow">${e.is_primary?'Main event':'Event'}</span><h3>${esc(e.name)}</h3><p><strong>${esc(e.event_date||'Date TBD')} ${esc(e.start_time||'')}</strong></p><p>${esc(e.venue||'Venue TBD')}</p><p>${esc(e.description||'')}</p>${e.rsvp_deadline?`<small>RSVP by ${esc(e.rsvp_deadline)}</small>`:''}</article>`).join('')||'<div class="empty-state">No events yet.</div>';$('[data-add-event]').onclick=()=>{const w=$('[data-event-form-wrap]');w.hidden=!w.hidden};$('[data-event-form]').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),body=Object.fromEntries(f.entries());body.is_primary=f.get('is_primary')==='on';await api('/api/events',{method:'POST',body:JSON.stringify(body)});location.reload()}}
